...
export class WorkItemService implements IWorkItemService {
	...
	async createWorkItem(
		...
		const hasAccess = await this.projectService.validateProjectAccess(
			projectId,
			userId,
		)
		if (!hasAccess) {
		...
		if (data.title.length > 200) {
			throw new ValidationError(
				'Work item title must be less than 200 characters',
			)
		}

		if (data.description && data.description.length > 2000) {
			throw new ValidationError(
				'Work item description must be less than 2000 characters',
			)
		}
		...
		if (
			...
			(data.storyPoints < 1 || data.storyPoints > 100)
		) {
		...
		if (
			...
			(data.estimatedHours < 1 || data.estimatedHours > 1000)
		) {
	...
	async updateWorkItemStatus(
		...
		if (!this.isValidStatusTransition(existingWorkItem.status, status)) {
			throw new BusinessLogicError(
				`Cannot transition from ${existingWorkItem.status} to ${status}`,
			)
		}

		try {
			const updatedWorkItem = await this.workItemRepository.update(id, {
				status,
			})
			return updatedWorkItem
		} catch (error) {
	...
	async getBacklog(projectId: string, userId: string): Promise<WorkItem[]> {
		...
		try {
			...
			const backlogItems =
				await this.workItemRepository.findBacklogItems(projectId)
	...
	async updatePriority(
		...
		try {
			...
			const updatedWorkItem = await this.workItemRepository.updatePriority(
				id,
				priorityOrder,
			)
			return updatedWorkItem
