  ...
  return new Elysia({ name: 'rate-limit' })
    .onRequest(({ request, set }) => {
      ...
      if (store[clientId].count >= maxRequests) {
        set.status = 429;
        set.headers['Retry-After'] = Math.ceil((store[clientId].resetTime - now) / 1000).toString();
        set.headers['X-RateLimit-Limit'] = maxRequests.toString();
        set.headers['X-RateLimit-Remaining'] = '0';
        set.headers['X-RateLimit-Reset'] = new Date(store[clientId].resetTime).toISOString();
      ...
      set.headers['X-RateLimit-Limit'] = maxRequests.toString();
      set.headers['X-RateLimit-Remaining'] = (maxRequests - store[clientId].count).toString();
      set.headers['X-RateLimit-Reset'] = new Date(store[clientId].resetTime).toISOString();
    })
