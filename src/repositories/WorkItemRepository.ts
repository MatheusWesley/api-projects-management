...
import type { WorkItem, CreateWorkItemData, UpdateWorkItemData } from '../types/workItem.js';
...
export class WorkItemRepository implements IWorkItemRepository {
  ...
  async create(itemData: CreateWorkItemData & { projectId: string; reporterId: string }): Promise<WorkItem> {
    const id = generateId();
    ...
    const maxPriorityStmt = this.db.prepare(`
      ...
      WHERE project_id = ?
    `);
    const maxPriorityResult = maxPriorityStmt.get(itemData.projectId) as { max_priority: number };
  ...
  async reorderBacklogItems(projectId: string, itemIds: string[]): Promise<void> {
  ...
  async getItemsWithAssigneeInfo(projectId: string): Promise<Array<WorkItem & { assigneeName?: string; assigneeEmail?: string }>> {
    const stmt = this.db.prepare(`
