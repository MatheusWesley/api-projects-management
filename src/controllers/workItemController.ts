			...
			new Elysia()
				...
				.get(
					...
					async ({ params, user, set }) => {
						try {
							...
							const workItems = await this.workItemService.getProjectWorkItems(
								projectId,
								user.userId,
							)
				...
				.post(
					...
					},
					{
						...
						body: t.Object({
							...
							type: t.Union(
								[t.Literal('task'), t.Literal('bug'), t.Literal('story')],
								{
				...
				.get(
					...
					async ({ params, user, set }) => {
						try {
							const { id } = params
							const workItem = await this.workItemService.getWorkItem(
								id,
								user.userId,
							)
				...
				.put(
					...
					async ({ params, body, user, set }) => {
						try {
							...
							const workItem = await this.workItemService.updateWorkItem(
								id,
								updateData,
								user.userId,
							)
				...
				.patch(
					...
					async ({ params, body, user, set }) => {
						try {
							...
							const workItem = await this.workItemService.updateWorkItemStatus(
								id,
								status,
								user.userId,
							)
					...
					},
					{
						...
						detail: {
							...
							description:
								'Update the status of a work item (for Kanban board movements)',
							tags: ['Work Items'],
						},
					},
				)
