...
export class AuthController {
  ...
  createRoutes() {
    return new Elysia({ prefix: '/auth' })
      .use(process.env.NODE_ENV !== 'test' ? createAuthRateLimit() : new Elysia())
      .post('/login', async ({ body, set }) => {
        ...
        response: {
          200: t.Object({
            ...
            data: t.Object({
              ...
              token: t.String({ examples: ['eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...'] })
            }),
        ...
        detail: {
          summary: 'User login',
          description: 'Authenticate user with email and password. Returns user information and JWT token for subsequent requests.',
          tags: ['Authentication'],
          examples: {
            'Valid login': {
      ...
      .post('/register', async ({ body, set }) => {
        ...
        detail: {
          ...
          description: 'Register a new user account. Email must be unique. Password will be securely hashed before storage.',
          tags: ['Authentication'],
          examples: {
            'New developer': {
