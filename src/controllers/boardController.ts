			...
			new Elysia()
				...
				.get(
					...
					async ({ params, user, set }) => {
						try {
							...
							const kanbanBoard = await this.workItemService.getKanbanBoard(
								projectId,
								user.userId,
							)
					...
					},
					{
						...
						detail: {
							...
							description:
								'Get Kanban board with work items grouped by status for a specific project',
							tags: ['Boards'],
						},
					},
				)
				...
				.get(
					...
					async ({ params, user, set }) => {
						try {
							const { projectId } = params
							const backlogItems = await this.workItemService.getBacklog(
								projectId,
								user.userId,
							)
					...
					},
					{
						...
						detail: {
							...
							description:
								'Get prioritized backlog items (todo status) for a specific project',
							tags: ['Boards'],
						},
					},
				)
				...
				.patch(
					...
					async ({ params, body, user, set }) => {
						try {
							...
							const updatedItem = await this.workItemService.updatePriority(
								id,
								priorityOrder,
								user.userId,
							)
					...
					},
					{
						...
						detail: {
							...
							description:
								'Update the priority order of a work item for backlog reordering',
							tags: ['Boards'],
						},
					},
				)
