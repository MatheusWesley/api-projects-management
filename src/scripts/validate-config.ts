...
function validateConfiguration(): ValidationResult {
	...
	if (isProduction()) {
		...
		if (
			...
			warnings.push(
				'Using default JWT_SECRET in development. Set JWT_SECRET in .env file',
			)
		}
	}
	...
	if (isProduction() && config.database.filename === './data/database.sqlite') {
		warnings.push(
			'Using default database path in production. Consider setting DB_PATH',
		)
	}

	if (
		config.database.filename !== ':memory:' &&
		!config.database.filename.includes('/')
	) {
		warnings.push(
			'Database path should be absolute or include directory structure',
		)
	}
	...
	if (isProduction() && config.bcryptRounds < 10) {
		warnings.push(
			'BCRYPT_ROUNDS should be at least 10 in production for security',
		)
	}
	...
	if (!validExpirationPattern.test(config.jwtExpiresIn)) {
		errors.push(
			`Invalid JWT_EXPIRES_IN format: ${config.jwtExpiresIn}. Use format like '24h', '7d', '30m'`,
		)
	}
...
function printValidationResults(result: ValidationResult) {
	...
	if (result.valid) {
		...
		console.log(
			'❌ Configuration has errors that must be fixed before starting the application.',
		)
	}
}
