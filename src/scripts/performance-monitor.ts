...
class PerformanceMonitor {
	...
	async runPerformanceTest(token?: string): Promise<void> {
		...
		for (const test of tests) {
			try {
				...
				console.log(
					`     Memory Delta: ${(metric.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)}MB\n`,
				)
			} catch (error) {
	...
	async runLoadTest(
		...
		console.log(
			`🔥 Running Load Test: ${concurrency} concurrent requests, ${requests} total requests\n`,
		)
	...
	generateReport(): void {
		...
		// Group by endpoint
		const endpointGroups = this.metrics.reduce(
			(groups, metric) => {
		...
		Object.entries(endpointGroups).forEach(([endpoint, metrics]) => {
			...
			const successRate =
				(metrics.filter((m) => m.statusCode >= 200 && m.statusCode < 400)
					.length /
					metrics.length) *
				100
		...
		const overallSuccess =
			(this.metrics.filter((m) => m.statusCode >= 200 && m.statusCode < 400)
				.length /
				this.metrics.length) *
			100
...
async function main() {
	...
	switch (command) {
		...
		default:
			console.log('Usage:')
			console.log(
				'  bun run src/scripts/performance-monitor.ts test [baseUrl] [token]',
			)
			console.log(
				'  bun run src/scripts/performance-monitor.ts load [baseUrl] [endpoint] [concurrency] [requests] [token]',
			)
			console.log(
				'  bun run src/scripts/performance-monitor.ts monitor [baseUrl] [token]',
			)
			break
	}
}
