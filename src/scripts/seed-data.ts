...
async function createSeedData(): Promise<{
	...
	const workItems: SeedWorkItem[] = [
		...
		},
		{
			...
			description:
				'Cart total is not updating correctly when items are removed',
			type: 'bug',
			status: 'todo',
			priority: 'critical',
			projectId: projects[0].id,
		...
		// Legacy System Migration items
		{
			...
			description:
				'Document current database structure and identify migration requirements',
			type: 'task',
			status: 'done',
			priority: 'high',
...
async function seedDatabase() {
	try {
		...
		if (!isDevelopment() && !isTest()) {
			console.log(
				'⚠️  Seeding is only allowed in development or test environments',
			)
			return
		}
    ...
    `)
		...
		users.forEach((user) => {
			console.log(
				`  - ${user.email} (${user.role}) - password: ${user.email.split('@')[0]}123`,
			)
		})
