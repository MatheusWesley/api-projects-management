...
async function setupEnvironment() {
  try {
    ...
    if (existsSync(envPath) && !force) {
      console.log('⚠️  .env file already exists. Use --force to overwrite or remove it manually.');
    ...
    if (environment === 'production') {
      ...
      console.log('3. Ensure DB_PATH directory exists and has proper permissions');
      console.log('4. Review all configuration values before deployment');
    } else if (environment === 'development') {
      console.log('\n💡 Development setup complete!');
      console.log('Run `bun run setup` to install dependencies and initialize database');
    }
