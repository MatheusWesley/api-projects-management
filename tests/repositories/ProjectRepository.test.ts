...
describe('ProjectRepository', () => {
	...
	describe('update', () => {
		...
		it('should throw error when updating non-existent project', async () => {
			await expect(
				projectRepository.update('non-existent-id', { name: 'New Name' }),
	...
	describe('isOwner', () => {
		it('should return true when user owns the project', async () => {
			...
			const isOwner = await projectRepository.isOwner(
				createdProject.id,
				testUserId,
			)
	...
	describe('findByIdWithOwner', () => {
		it('should find project with owner information', async () => {
			...
			const projectWithOwner = await projectRepository.findByIdWithOwner(
				createdProject.id,
			)
		...
		it('should return null for non-existent project', async () => {
			const projectWithOwner =
				await projectRepository.findByIdWithOwner('non-existent-id')
