...
export function createTestUser(db: any, overrides: Partial<typeof testUserData> = {}) {
  ...
  stmt.run(id, userData.email, userData.name, userData.password, userData.role, now, now);
  return id;
}

export function createTestProject(db: any, ownerId: string, overrides: Partial<typeof testProjectData> = {}) {
...
export function createTestWorkItem(
  ...
  stmt.run(id, itemData.title, itemData.description, itemData.type, projectId, reporterId, now, now);
  return id;
}