...
export class MockDatabase {
  ...
  private executeQuery(sql: string, params: any[] = []): any {
    ...
    } else if (normalizedSql.includes('select owner_id from projects where id')) {
    ...
    } else if (normalizedSql.includes('select * from projects') && normalizedSql.includes('where owner_id')) {
    ...
    } else if (normalizedSql.includes('select * from work_items') && normalizedSql.includes('project_id = ?') && normalizedSql.includes('status = ?') && params.length === 2) {
      return this.findWorkItemsByProjectAndStatus(params[0], params[1]);
    } else if (normalizedSql.includes('select * from work_items where project_id = ?') && params.length > 1) {
      return this.findWorkItemsWithFilters(sql, params);
    } else if (normalizedSql.includes('select * from work_items') && normalizedSql.includes('where assignee_id = ?')) {
      return this.findWorkItemsByAssigneeId(params[0]);
    } else if (normalizedSql.includes('select * from work_items') && normalizedSql.includes('where project_id') && normalizedSql.includes("status = 'todo'")) {
      return this.findBacklogItems(params[0]);
    } else if (normalizedSql.includes('select * from work_items') && normalizedSql.includes('where project_id')) {
    ...
    } else if (normalizedSql.includes('update work_items') && normalizedSql.includes('and project_id = ?')) {
    ...
    } else if (normalizedSql.includes('select p.*, u.name as owner_name, u.email as owner_email')) {
  ...
  private insertProject(params: any[]) {
    ...
    const adjustedCreatedAt = new Date(new Date(created_at).getTime() + existingProjects).toISOString();
    const adjustedUpdatedAt = new Date(new Date(updated_at).getTime() + existingProjects).toISOString();

    const project = { id, name, description, owner_id, status: 'active', created_at: adjustedCreatedAt, updated_at: adjustedUpdatedAt };
  ...
  private insertWorkItem(params: any[]) {
    ...
    const [id, title, description, type, priority, project_id, assignee_id, reporter_id, story_points, estimated_hours, priority_order, created_at, updated_at] = params;
    ...
    const workItem = { 
      id, title, description, type, status: 'todo', priority,
      project_id, assignee_id, reporter_id, story_points, estimated_hours, 
  ...
  private findWorkItemsByProjectId(projectId: string) {
    const workItems = this.tables.get('work_items')!;
    return workItems.filter(w => w.project_id === projectId)
      .sort((a, b) => a.priority_order - b.priority_order);
  }

  private findWorkItemsByAssigneeId(assigneeId: string) {
    const workItems = this.tables.get('work_items')!;
    return workItems.filter(w => w.assignee_id === assigneeId)
      .sort((a, b) => a.priority_order - b.priority_order);
  }
  ...
  private updateUser(sql: string, params: any[]) {
    ...
    for (const update of updates) {
      if (update.includes('email =')) {
        ...
        user.updated_at = newUpdatedAt.getTime() <= createdAt.getTime() 
  ...
  private updateProject(sql: string, params: any[]) {
    ...
    project.updated_at = newUpdatedAt.getTime() <= createdAt.getTime() 
      ? new Date(createdAt.getTime() + 1).toISOString()
      : newUpdatedAt.toISOString();
  ...
  private updateWorkItem(sql: string, params: any[]) {
    ...
    item.updated_at = newUpdatedAt.getTime() <= createdAt.getTime() 
      ? new Date(createdAt.getTime() + 1).toISOString()
      : newUpdatedAt.toISOString();
    return { changes: 1 };
  }
  ...
  private getMaxPriorityOrder(projectId: string) {
    ...
    const maxPriority = projectItems.length > 0 
  ...
  private findBacklogItems(projectId: string) {
    const workItems = this.tables.get('work_items')!;
    return workItems.filter(w => w.project_id === projectId && w.status === 'todo');
  }
  ...
  private findWorkItemsByProjectAndStatus(projectId: string, status: string) {
    const workItems = this.tables.get('work_items')!;
    return workItems.filter(w => w.project_id === projectId && w.status === status)
      .sort((a, b) => a.priority_order - b.priority_order);
  }
  ...
  private findWorkItemsWithFilters(sql: string, params: any[]) {
    ...
    if (lowerSql.includes('and status = ?')) {
      filteredItems = filteredItems.filter(w => w.status === params[paramIndex++]);
    }
    if (lowerSql.includes('and type = ?')) {
      filteredItems = filteredItems.filter(w => w.type === params[paramIndex++]);
    }
    if (lowerSql.includes('and assignee_id = ?')) {
      filteredItems = filteredItems.filter(w => w.assignee_id === params[paramIndex++]);
    }
    if (lowerSql.includes('and priority = ?')) {
      filteredItems = filteredItems.filter(w => w.priority === params[paramIndex++]);
    }
  ...
  private updateWorkItemPriority(params: any[]) {
    ...
    const itemIndex = workItems.findIndex(w => w.id === itemId && w.project_id === projectId);
    if (itemIndex === -1) {
      return { changes: 0 };
    }
