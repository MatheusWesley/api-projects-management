...
describe('ProjectService', () => {
	...
	beforeEach(() => {
		...
		mockProjectRepository = {
			...
			update: mock(() =>
				Promise.resolve({ ...mockProject, ...mockUpdateProjectData }),
			),
	...
	describe('createProject', () => {
		...
		it('should throw ValidationError when name is missing', async () => {
			...
			await expect(
				projectService.createProject(invalidData, 'user1'),
			).rejects.toThrow(ValidationError)
		})

		it('should throw ValidationError when userId is missing', async () => {
			await expect(
				projectService.createProject(mockCreateProjectData, ''),
			).rejects.toThrow(ValidationError)
		})
		...
		it('should throw ValidationError when name is too long', async () => {
			...
			await expect(
				projectService.createProject(longNameData, 'user1'),
			).rejects.toThrow(ValidationError)
		})
		...
		it('should throw ValidationError when description is too long', async () => {
			...
			await expect(
				projectService.createProject(longDescData, 'user1'),
			).rejects.toThrow(ValidationError)
		})

		it('should throw BusinessLogicError when repository fails', async () => {
			mockProjectRepository.create = mock(() =>
				Promise.reject(new Error('Database error')),
			)

			await expect(
				projectService.createProject(mockCreateProjectData, 'user1'),
	...
	describe('getProject', () => {
		...
		it('should throw ValidationError when id is missing', async () => {
			await expect(projectService.getProject('', 'user1')).rejects.toThrow(
				ValidationError,
			)
		})

		it('should throw ValidationError when userId is missing', async () => {
			await expect(projectService.getProject('1', '')).rejects.toThrow(
				ValidationError,
			)
		})
		...
		it('should throw NotFoundError when project does not exist', async () => {
			...
			await expect(projectService.getProject('1', 'user1')).rejects.toThrow(
				NotFoundError,
			)
		})
		...
		it('should throw ForbiddenError when user does not have access', async () => {
			...
			mockProjectRepository.findById = mock(() =>
				Promise.resolve(otherUserProject),
			)
	...
	describe('updateProject', () => {
		...
		it('should throw ValidationError when id is missing', async () => {
			await expect(
				projectService.updateProject('', mockUpdateProjectData, 'user1'),
			).rejects.toThrow(ValidationError)
		})

		it('should throw ValidationError when userId is missing', async () => {
			await expect(
				projectService.updateProject('1', mockUpdateProjectData, ''),
			).rejects.toThrow(ValidationError)
		})
		...
		it('should throw ForbiddenError when user is not owner', async () => {
			...
			await expect(
				projectService.updateProject('1', mockUpdateProjectData, 'user1'),
		...
		it('should throw ValidationError when name is empty', async () => {
			...
			await expect(
				projectService.updateProject('1', invalidData, 'user1'),
			).rejects.toThrow(ValidationError)
		})
	...
	describe('deleteProject', () => {
		...
		it('should throw ValidationError when id is missing', async () => {
			await expect(projectService.deleteProject('', 'user1')).rejects.toThrow(
				ValidationError,
			)
		})
		...
		it('should throw ForbiddenError when user is not owner', async () => {
			...
			await expect(projectService.deleteProject('1', 'user1')).rejects.toThrow(
				ForbiddenError,
			)
		})
	})
	...
	describe('listUserProjects', () => {
		...
		it('should throw ValidationError when userId is missing', async () => {
			await expect(projectService.listUserProjects('')).rejects.toThrow(
				ValidationError,
			)
		})

		it('should throw BusinessLogicError when repository fails', async () => {
			mockProjectRepository.findByUserId = mock(() =>
				Promise.reject(new Error('Database error')),
			)

			await expect(projectService.listUserProjects('user1')).rejects.toThrow(
				BusinessLogicError,
			)
		})
	})
	...
	describe('validateProjectAccess', () => {
		...
		it('should return false when projectId or userId is missing', async () => {
			expect(await projectService.validateProjectAccess('', 'user1')).toBe(
				false,
			)
