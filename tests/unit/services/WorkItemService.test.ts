...
import type { WorkItem, CreateWorkItemData, UpdateWorkItemData, KanbanBoard } from '../../../src/types/workItem.js';
import { 
  ValidationError, 
  NotFoundError, 
  ForbiddenError,
...
describe('WorkItemService', () => {
  ...
  beforeEach(() => {
    ...
    mockWorkItemRepository = {
      ...
      update: mock(() => Promise.resolve({ ...mockWorkItem, ...mockUpdateWorkItemData })),
      delete: mock(() => Promise.resolve()),
      findByStatus: mock(() => Promise.resolve([mockWorkItem])),
      updatePriority: mock(() => Promise.resolve({ ...mockWorkItem, priorityOrder: 2 })),
    ...
    workItemService = new WorkItemService(mockWorkItemRepository, mockProjectService);
  });

  describe('createWorkItem', () => {
    it('should create a work item successfully', async () => {
      const result = await workItemService.createWorkItem(mockCreateWorkItemData, 'project1', 'user1');

      expect(result).toEqual(mockWorkItem);
      expect(mockProjectService.validateProjectAccess).toHaveBeenCalledWith('project1', 'user1');
    ...
    it('should throw ValidationError when title is missing', async () => {
      ...
      await expect(workItemService.createWorkItem(invalidData, 'project1', 'user1')).rejects.toThrow(ValidationError);
    });

    it('should throw ValidationError when projectId is missing', async () => {
      await expect(workItemService.createWorkItem(mockCreateWorkItemData, '', 'user1')).rejects.toThrow(ValidationError);
    });

    it('should throw ValidationError when userId is missing', async () => {
      await expect(workItemService.createWorkItem(mockCreateWorkItemData, 'project1', '')).rejects.toThrow(ValidationError);
    });

    it('should throw ForbiddenError when user does not have project access', async () => {
      mockProjectService.validateProjectAccess = mock(() => Promise.resolve(false));
      
      await expect(workItemService.createWorkItem(mockCreateWorkItemData, 'project1', 'user1')).rejects.toThrow(ForbiddenError);
    });
    ...
    it('should throw ValidationError when description is too long', async () => {
      const invalidData = { ...mockCreateWorkItemData, description: 'a'.repeat(2001) };
      await expect(workItemService.createWorkItem(invalidData, 'project1', 'user1')).rejects.toThrow(ValidationError);
    });
  ...
  describe('getWorkItem', () => {
    ...
    it('should throw ValidationError when id is missing', async () => {
      await expect(workItemService.getWorkItem('', 'user1')).rejects.toThrow(ValidationError);
    });

    it('should throw ValidationError when userId is missing', async () => {
      await expect(workItemService.getWorkItem('1', '')).rejects.toThrow(ValidationError);
    });
    ...
    it('should throw NotFoundError when work item does not exist', async () => {
      ...
      await expect(workItemService.getWorkItem('1', 'user1')).rejects.toThrow(NotFoundError);
    });

    it('should throw ForbiddenError when user does not have project access', async () => {
      mockProjectService.validateProjectAccess = mock(() => Promise.resolve(false));
      
      await expect(workItemService.getWorkItem('1', 'user1')).rejects.toThrow(ForbiddenError);
    });
  });

  describe('updateWorkItem', () => {
    it('should update a work item successfully', async () => {
      const result = await workItemService.updateWorkItem('1', mockUpdateWorkItemData, 'user1');

      expect(result).toEqual({ ...mockWorkItem, ...mockUpdateWorkItemData });
      expect(mockWorkItemRepository.update).toHaveBeenCalledWith('1', mockUpdateWorkItemData);
    });

    it('should throw ValidationError when title is empty', async () => {
      const invalidData = { title: '' };
      await expect(workItemService.updateWorkItem('1', invalidData, 'user1')).rejects.toThrow(ValidationError);
    });
  ...
  describe('deleteWorkItem', () => {
    ...
    it('should throw ValidationError when id is missing', async () => {
      await expect(workItemService.deleteWorkItem('', 'user1')).rejects.toThrow(ValidationError);
    });
  });

  describe('getProjectWorkItems', () => {
    it('should get project work items successfully', async () => {
      const result = await workItemService.getProjectWorkItems('project1', 'user1');

      expect(result).toEqual([mockWorkItem]);
      expect(mockProjectService.validateProjectAccess).toHaveBeenCalledWith('project1', 'user1');
      expect(mockWorkItemRepository.findByProjectId).toHaveBeenCalledWith('project1');
    });

    it('should throw ForbiddenError when user does not have project access', async () => {
      mockProjectService.validateProjectAccess = mock(() => Promise.resolve(false));
      
      await expect(workItemService.getProjectWorkItems('project1', 'user1')).rejects.toThrow(ForbiddenError);
    });
  });
  ...
  describe('getKanbanBoard', () => {
    it('should get Kanban board successfully', async () => {
      ...
      const inProgressItem = { ...mockWorkItem, id: '2', status: 'in_progress' as const };
      const doneItem = { ...mockWorkItem, id: '3', status: 'done' as const };
      
      mockWorkItemRepository.findByProjectId = mock(() => Promise.resolve([todoItem, inProgressItem, doneItem]));
    ...
    it('should throw ForbiddenError when user does not have project access', async () => {
      ...
      await expect(workItemService.getKanbanBoard('project1', 'user1')).rejects.toThrow(ForbiddenError);
    });
  });
  ...
  describe('updateWorkItemStatus', () => {
    it('should update work item status successfully', async () => {
      ...
      mockWorkItemRepository.update = mock(() => Promise.resolve({ ...mockWorkItem, status: 'in_progress' }));
      
      const result = await workItemService.updateWorkItemStatus('1', 'in_progress', 'user1');

      expect(result.status).toBe('in_progress');
      expect(mockWorkItemRepository.update).toHaveBeenCalledWith('1', { status: 'in_progress' });
    });

    it('should throw ValidationError when status is invalid', async () => {
      await expect(workItemService.updateWorkItemStatus('1', 'invalid' as any, 'user1')).rejects.toThrow(ValidationError);
    });
    ...
    it('should throw BusinessLogicError when status transition is invalid', async () => {
      ...
      await expect(workItemService.updateWorkItemStatus('1', 'todo', 'user1')).rejects.toThrow(BusinessLogicError);
    });
  });
  ...
  describe('getBacklog', () => {
    it('should get backlog successfully', async () => {
      ...
      expect(result).toEqual([mockWorkItem]);
      expect(mockProjectService.validateProjectAccess).toHaveBeenCalledWith('project1', 'user1');
      expect(mockWorkItemRepository.findBacklogItems).toHaveBeenCalledWith('project1');
    });

    it('should throw ForbiddenError when user does not have project access', async () => {
      mockProjectService.validateProjectAccess = mock(() => Promise.resolve(false));
      
      await expect(workItemService.getBacklog('project1', 'user1')).rejects.toThrow(ForbiddenError);
    });
  });
  ...
  describe('updatePriority', () => {
    ...
    it('should throw ValidationError when priority order is negative', async () => {
      await expect(workItemService.updatePriority('1', -1, 'user1')).rejects.toThrow(ValidationError);
    });
  });
  ...
  describe('assignWorkItem', () => {
    it('should assign work item successfully', async () => {
      ...
      mockWorkItemRepository.update = mock(() => Promise.resolve({ ...mockWorkItem, assigneeId: 'user2' }));
      
      const result = await workItemService.assignWorkItem('1', 'user2', 'user1');

      expect(result.assigneeId).toBe('user2');
      expect(mockWorkItemRepository.update).toHaveBeenCalledWith('1', { assigneeId: 'user2' });
    });

    it('should throw ValidationError when assigneeId is missing', async () => {
      await expect(workItemService.assignWorkItem('1', '', 'user1')).rejects.toThrow(ValidationError);
    });
  });
});