	...
	describe('Error Message Quality', () => {
		it('should provide detailed error messages for validation failures', () => {
			...
			try {
				...
				const errorPaths = validationError.details.errors.map(
					(e: any) => e.path,
				)
				expect(errorPaths.some((path: string) => path.includes('email'))).toBe(
					true,
				)
				expect(errorPaths.some((path: string) => path.includes('name'))).toBe(
					true,
				)
				expect(
					errorPaths.some((path: string) => path.includes('password')),
				).toBe(true)
				expect(errorPaths.some((path: string) => path.includes('role'))).toBe(
					true,
				)
			}
		})
