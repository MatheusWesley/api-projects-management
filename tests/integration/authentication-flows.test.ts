  ...
  describe('Invalid Authentication Flows', () => {
    it('should reject invalid registration data', async () => {
      const invalidRegistrations = [
        ...
        {
          data: { email: 'invalid-email', name: 'Test', password: 'Password123!', role: 'developer' },
          expectedStatus: 422
        },
        // Missing name
        {
          data: { email: 'test@example.com', password: 'Password123!', role: 'developer' },
          expectedStatus: 422
        },
        // Empty name
        {
          data: { email: 'test@example.com', name: '', password: 'Password123!', role: 'developer' },
          expectedStatus: 422
        },
        // Missing password
        {
        ...
        // Password too short
        {
          data: { email: 'test@example.com', name: 'Test', password: '123', role: 'developer' },
          expectedStatus: 422
        },
        // Missing role
        {
        ...
        // Invalid role
        {
          data: { email: 'test@example.com', name: 'Test', password: 'Password123!', role: 'invalid' },
          expectedStatus: 422
        }
      ];
    ...
    it('should reject invalid tokens for protected routes', async () => {
      const invalidTokenScenarios = [
        ...
        // Wrong scheme
        { headers: { 'Authorization': 'Basic dXNlcjpwYXNz' }, expectedStatus: 401 },
        // Malformed Bearer token
        { headers: { 'Authorization': 'Bearer' }, expectedStatus: 401 },
        // Invalid token format
        { headers: { 'Authorization': 'Bearer invalid-token' }, expectedStatus: 401 },
        // Empty token
        { headers: { 'Authorization': 'Bearer ' }, expectedStatus: 401 },
        // Token for non-existent user
        { headers: { 'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJub24tZXhpc3RlbnQiLCJpYXQiOjE2MzQ1NjcwMDB9.invalid' }, expectedStatus: 401 }
      ];
  ...
  describe('Token Validation and Security', () => {
    ...
    it('should handle authentication with different HTTP methods', async () => {
      ...
      const protectedEndpoints = [
        ...
        { method: 'POST', url: 'http://localhost/projects', body: { name: 'Test', description: 'Test' } },
        { method: 'GET', url: `http://localhost/projects/${testProject.project.id}` },
        { method: 'PUT', url: `http://localhost/projects/${testProject.project.id}`, body: { name: 'Updated' } },
        { method: 'DELETE', url: `http://localhost/projects/${testProject.project.id}` }
      ];
  ...
  describe('Password Security', () => {
    it('should handle various password formats', async () => {
      const passwordTests = [
        ...
        { password: 'VeryLongPasswordThatExceedsNormalLengthButShouldStillWork123!', shouldWork: true },
