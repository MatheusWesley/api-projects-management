...
describe('Authorization Scenarios Integration Tests', () => {
	...
	describe('Project Access Authorization', () => {
		...
		it('should only show user their own projects in list', async () => {
			...
			const user1Project1 = await testFactory.createTestProject(user1, {
				name: 'User 1 Project 1',
			})
			const user1Project2 = await testFactory.createTestProject(user1, {
				name: 'User 1 Project 2',
			})
			const user2Project1 = await testFactory.createTestProject(user2, {
				name: 'User 2 Project 1',
			})
			const user2Project2 = await testFactory.createTestProject(user2, {
				name: 'User 2 Project 2',
			})
			...
			const user1ProjectNames = user1ProjectsResult.data.projects.map(
				(p: any) => p.name,
			)
			...
			const user2ProjectNames = user2ProjectsResult.data.projects.map(
				(p: any) => p.name,
			)
	...
	describe('Work Item Access Authorization', () => {
		...
		it('should only show work items from accessible projects', async () => {
			...
			const user1Item1 = await testFactory.createTestWorkItem(
				user1Project,
				user1,
				{ title: 'User 1 Item 1' },
			)
			const user1Item2 = await testFactory.createTestWorkItem(
				user1Project,
				user1,
				{ title: 'User 1 Item 2' },
			)
			const user2Item1 = await testFactory.createTestWorkItem(
				user2Project,
				user2,
				{ title: 'User 2 Item 1' },
			)
			...
			const user1ItemsResponse = await app.handle(
				new Request(
					`http://localhost/projects/${user1Project.project.id}/items`,
					{
						method: 'GET',
						headers: { Authorization: `Bearer ${user1.token}` },
					},
			...
			const user1ItemTitles = user1ItemsResult.data.workItems.map(
				(item: any) => item.title,
			)
			...
			const unauthorizedItemsResult =
				(await unauthorizedItemsResponse.json()) as any
	...
	describe('Board and Backlog Access Authorization', () => {
		it('should allow project owners to access Kanban board and backlog', async () => {
			...
			await testFactory.createTestWorkItem(project, owner, {
				title: 'Todo Item',
				type: 'task',
			})
			const inProgressItem = await testFactory.createTestWorkItem(
				project,
				owner,
				{ title: 'In Progress Item', type: 'bug' },
			)

			// Move one item to in_progress
			await app.handle(
				new Request(
					`http://localhost/items/${inProgressItem.workItem.id}/status`,
					{
						method: 'PATCH',
						headers: {
							Authorization: `Bearer ${owner.token}`,
	...
	describe('Cross-User Authorization Scenarios', () => {
		it('should handle complex multi-user authorization scenarios', async () => {
			...
			const devProject = await testFactory.createTestProject(developer, {
				name: 'Dev Project',
			})
			const managerProject = await testFactory.createTestProject(manager, {
				name: 'Manager Project',
			})
			const adminProject = await testFactory.createTestProject(admin, {
				name: 'Admin Project',
			})

			// Create work items in each project
			const devItem = await testFactory.createTestWorkItem(
				devProject,
				developer,
			)
			const managerItem = await testFactory.createTestWorkItem(
				managerProject,
				manager,
			)
			const adminItem = await testFactory.createTestWorkItem(
				adminProject,
				admin,
			)
		...
		it('should prevent privilege escalation attempts', async () => {
			...
			const adminProject = await testFactory.createTestProject(adminUser, {
				name: 'Admin Project',
			})
			...
			const privilegeEscalationAttempts = [
				{
					method: 'GET',
					url: `http://localhost/projects/${adminProject.project.id}`,
				},
				{
					method: 'PUT',
					url: `http://localhost/projects/${adminProject.project.id}`,
				...
				{
					method: 'POST',
					url: `http://localhost/projects/${adminProject.project.id}/items`,
					body: { title: 'Hack', type: 'task' },
				},
				{
					method: 'GET',
					url: `http://localhost/projects/${adminProject.project.id}/kanban`,
				},
				{
					method: 'GET',
					url: `http://localhost/projects/${adminProject.project.id}/backlog`,
				},
			]
	...
	describe('Resource Ownership Validation', () => {
		it('should validate ownership at every level of nested resources', async () => {
			...
			const ownerWorkItem = await testFactory.createTestWorkItem(
				ownerProject,
				owner,
			)
			const attackerWorkItem = await testFactory.createTestWorkItem(
				attackerProject,
				attacker,
			)
			...
			const unauthorizedAccess = [
				...
				{
					url: `http://localhost/items/${ownerWorkItem.workItem.id}`,
					method: 'GET',
				},
				{
					url: `http://localhost/items/${ownerWorkItem.workItem.id}`,
				...
				{
					url: `http://localhost/items/${ownerWorkItem.workItem.id}/status`,
					method: 'PATCH',
					body: { status: 'done' },
				},
				{
					url: `http://localhost/items/${ownerWorkItem.workItem.id}/priority`,
					method: 'PATCH',
				...
				{
					url: `http://localhost/projects/${ownerProject.project.id}/items`,
					method: 'GET',
				},
				{
					url: `http://localhost/projects/${ownerProject.project.id}/items`,
				...
				// Board access
				{
					url: `http://localhost/projects/${ownerProject.project.id}/kanban`,
					method: 'GET',
				},
				{
					url: `http://localhost/projects/${ownerProject.project.id}/backlog`,
					method: 'GET',
				},
			]
		...
		it('should prevent resource enumeration attacks', async () => {
			...
			const user1WorkItem = await testFactory.createTestWorkItem(
				user1Project,
				user1,
			)
			const user2WorkItem = await testFactory.createTestWorkItem(
				user2Project,
				user2,
			)
