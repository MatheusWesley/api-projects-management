...
export class TestDatabaseSetup {
	 ...
	 */
	async createContextWithTestData(): Promise<
		TestDatabaseContext & {
			testData: {
	...
	> {
		...
		for (const config of workItemConfigs) {
			const workItem = await workItemService.createWorkItem(
				...
				projects[config.projectIndex].id,
				users[config.reporterIndex].user.id,
			)
	...
	async verifyDatabaseState(context: TestDatabaseContext): Promise<{
		...
		const { userRepository, projectRepository, workItemRepository } =
	...
	resetDatabase(context: TestDatabaseContext): void {
		...
		context.repositories.projectRepository = new ProjectRepository(
			newMockDb as any,
		)
		context.repositories.workItemRepository = new WorkItemRepository(
			newMockDb as any,
		)

		// Recreate services with new repositories
		context.services.authService = new AuthService(
			context.repositories.userRepository,
		)
		context.services.projectService = new ProjectService(
	...
	simulateDatabaseError(
		...
		errorType: 'connection' | 'timeout' | 'constraint',
	): void {
