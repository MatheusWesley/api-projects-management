...
export class TestDataFactory {
  ...
  async createTestProject(owner: TestUser, overrides: Partial<{
    name: string;
    description: string;
  }> = {}): Promise<TestProject> {
    const projectData = {
    ...
    const project = await this.projectService.createProject(projectData, owner.user.id);
  ...
  async createCompleteTestSetup(): Promise<{
    ...
    const inProgressBug = await this.createTestWorkItem(mainProject, developer, {
...
export function expectErrorResponse(response: any, expectedCode?: string, expectedMessage?: string) {
  expect(response).toHaveProperty('success');
