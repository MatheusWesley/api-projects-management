...
describe('Work Item Integration Tests', () => {
	...
	beforeEach(async () => {
		...
		const anotherLoginResult = await authService.login(
			'another@example.com',
			'Password123!',
		)
	...
	describe('POST /projects/:projectId/items', () => {
		it('should create work item successfully with valid data', async () => {
			...
			expect(result.data.workItem.estimatedHours).toBe(
				workItemData.estimatedHours,
			)
	...
	describe('End-to-End Workflow', () => {
		it('should support complete work item lifecycle', async () => {
			...
			expect(updateResult.data.workItem.title).toBe(
				'Updated Lifecycle Test Item',
			)
