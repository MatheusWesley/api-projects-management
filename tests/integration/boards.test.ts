...
import type { IWorkItemRepository, IProjectRepository, IUserRepository } from '../../src/types/repositories.js';
import type { IWorkItemService, IProjectService, IAuthService } from '../../src/types/services.js';
...
describe('Board and Backlog Integration Tests', () => {
  ...
  describe('GET /projects/:projectId/kanban', () => {
    ...
    it('should return Kanban board with work items grouped by status', async () => {
      ...
      // Update statuses
      await workItemService.updateWorkItemStatus(inProgressItem.id, 'in_progress', testUser.id);
      await workItemService.updateWorkItemStatus(doneItem.id, 'done', testUser.id);
      ...
      expect(result.data.kanbanBoard.in_progress[0].title).toBe('In Progress Item');
  ...
  describe('PATCH /items/:id/priority', () => {
    ...
    it('should return 403 when user does not have access to work item', async () => {
      ...
      const anotherWorkItem = await workItemService.createWorkItem({
        ...
        type: 'task'
      }, anotherProject.id, anotherUser.id);
